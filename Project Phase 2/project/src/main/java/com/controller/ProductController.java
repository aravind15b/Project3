...
public class ProductController {
	...
	public String gotoProduct(Product product, Model m) {
		...
		m.addAttribute("supdetail",this.getSupList());
		m.addAttribute("proddetail",productDaoImpl.retrive());
	...
	public LinkedHashMap<Integer, String> getSupList() {
		List<Supplier> list = supplierDaoImpl.retrive();
...
public String ProductDesc(Model m,@PathVariable("proid")int proid) {
	m.addAttribute("product",productDaoImpl.getProduct(proid));
	return "ProductDesc";
}
	@RequestMapping(value = "/Product1/{proid}")
